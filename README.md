...
On Vercel the app runs as a static page + a serverless function, configured by
`vercel.json`:

- `index.html` is served by Vercel's static layer.
- `api/generate.js` runs as a serverless function at `/api/generate` (it shares
  all logic with local dev via `lib/generate.js`).
- `server.js` is used only for local `npm start`.
...
- **`server.js`** — local [Express](https://expressjs.com) server (`npm start`)
  that serves the page and calls `lib/generate.js`.
- **`api/generate.js`** — the Vercel serverless function for `/api/generate`,
  also calling `lib/generate.js`. Wired up by `vercel.json`.
